...
func _ready() :
	red_circle = 0
	self.connect("instance_clover_tree", get_tree().root.get_node("Game/game_start"), "_on_clover_tree_item_pressed")
...
func _on_TextureButton_pressed():
	if 	red_circle == 0 :
		emit_signal("instance_clover_tree")
