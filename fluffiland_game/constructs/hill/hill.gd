...
func _ready():
	...
	self.connect("diversity_level_2_enter", get_tree().root.get_node("Game/game_start"), "_on_diversity_level_2_enter")
	self.connect("diversity_level_2_out", get_tree().root.get_node("Game/game_start"), "_on_diversity_level_2_out")
...
func _on_Timer_timeout():
	...
	if health_time >= 60 :
		...
		if nutrient > 2 :
			...
			self.queue_free()
			emit_signal("diversity_level_2_out")
