...
signal ancistrus_birth
signal ancistrus_death
signal gender
...
onready var poop_scene = load("res://food/vegetals/bush/bush.tscn")
...
func _ready():
	...
	self.connect("ancistrus_birth", get_tree().root.get_node("Game/game_start"), "_on_ancistrus_birth")
	self.connect("ancistrus_death", get_tree().root.get_node("Game/game_start"), "_on_ancistrus_death")
	
	
	
	emit_signal("ancistrus_birth")
...
func set_interest():
	...
	for i in num_rays:
		...
		if result :
			...
			elif energy < hunger and target.is_in_group("poop") and not predator_here and not love_here :
				randomize()
				interest[i] = 1.0 + float (look_ahead) / float (distance) # (randi()*1.0 + 0.1 ) * ((1 / distance))
...
func _on_poop_timeout():
	if food_eaten == true :
		...
		if sprite_direction == "right" :
			poop.position.x = self.position.x - 90
			poop.position.y = self.position.y - 30
