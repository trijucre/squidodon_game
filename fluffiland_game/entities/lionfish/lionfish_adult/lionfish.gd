...
var hunger = 50
var target
var food_needed_birth = 30
var food_needed_birth_base = 30
...
func _ready() :
	...
	add_to_group("class_4", true)
	add_to_group("animal", true)
	add_to_group("predator", true)
	add_to_group("lionfish", true)
	
	self.connect("lionfish", get_tree().root.get_node("Game"), "_on_lionfish_lionfish_birth")
	self.connect("lionfish", get_tree().root.get_node("Game"), "_lionfish_lionfish_death")
	
	emit_signal("lionfish_birth")
...
func _on_fare_vision_body_entered(body):
	if  not body.is_in_group("lionfish") :
		if energy <= hunger :
			state = states.HUNT
			target = body
...
func _on_flee_area_tiny_body_entered(body):
	if not body.is_in_group("lionfish") and body.is_in_group("predator"):
		state = states.DEFAULT
		target = body
