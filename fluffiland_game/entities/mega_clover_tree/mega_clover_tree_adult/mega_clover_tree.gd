...
var center_y = self.position.y - 200
var spawn_area : Rect2 = Rect2(center_x, center_y, 400, 400)
...
func _on_Timer_timeout():
	...
	if bush_time >= 60 and health == health_max :
		...
		if bush_produced <= 30 :
			...
			tree_bush.position.x = self.position.x - 200 + rngx.randf_range(0, spawn_area.size.x)
			tree_bush.position.y = self.position.y - 200 + rngy.randf_range(0, spawn_area.size.x)
	...
	if health_time >= 60 :
		...
		if 	get_tree().root.get_node("Game/game_start").water_count >= 15 :
			emit_signal("water_spend", 15)
			health += 1
