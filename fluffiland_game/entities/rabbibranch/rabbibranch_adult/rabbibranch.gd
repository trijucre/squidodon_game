...
signal rabbibranch_birth
signal rabbibranch_death
signal gender
...
#variable childbirth
var egg_scene = preload("res://entities/rabbibranch/rabbibranch_egg/rabbibranch_egg.tscn")
var egg_number = 3
...
func _on_pregnancy_timeout():
	...
	for i in egg_number :
		...
		if sprite_direction == "left" :
			egg.position.x = self.position.x + 50 + randi () % -70 + 70
			egg.position.y = self.position.y + randi () % -70 + 70
